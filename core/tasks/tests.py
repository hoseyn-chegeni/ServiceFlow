...
class TaskStatusModelTest(TestCase):
    def setUp(self):
        ...
        self.status = TaskStatus.objects.create(
            name="Test Status", created_by=self.user, description="Test Description"
        )
...
class TaskStatusViewTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            email="admin@amdin.com", password="testpassword"
        )
        self.status = TaskStatus.objects.create(
            name="Test Status", created_by=self.user, description="Test Description"
        )
    ...
    def test_task_status_detail_view(self):
        response = self.client.get(
            reverse("tasks:detail_status", args=[self.status.pk])
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Test Status")
    ...
    def test_task_status_update_view(self):
        response = self.client.get(
            reverse("tasks:update_status", args=[self.status.pk])
        )
        self.assertEqual(response.status_code, 200)

    def test_task_status_delete_view(self):
        response = self.client.get(
            reverse("tasks:delete_status", args=[self.status.pk])
        )
        self.assertEqual(response.status_code, 200)
