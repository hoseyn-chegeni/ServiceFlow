...
from .models import TaskStatusChange, TaskPriorityChange, TaskAssignmentHistory
from db_events.models import TaskLog

@receiver(post_save, sender=TaskStatusChange)
def log_task_status(sender, instance, created, **kwargs):
    if created:
        TaskLog.objects.create(
            user=instance.changed_by,
            task = instance.task,
            event_type="Change Status",
            additional_info=f"{instance.changed_by} Set '{instance.status}' for {instance.task}",
        )
